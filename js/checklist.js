...
// Speicher-Schlüssel
const CHECKED_STATE_KEY = 'focusDayChecklistState_v19'; 
...
function isCategoryInTimeWindow(key) {
    const cat = currentChecklistData[key];
    if (!cat || !cat.autoSelect) return false;
...
function renderTabs() {
    ...
    tabButtons.forEach(btn => {
        ...
        // Active State Styling (Deep Focus Theme)
        if (key === activeTabId) {
            btn.classList.add('border-teal-600', 'text-slate-100');
            btn.classList.remove('border-transparent', 'text-slate-500');
        } else {
            btn.classList.remove('border-teal-600', 'text-slate-100');
...
function renderActiveList() {
    ...
    category.tasks.forEach(task => {
        ...
        // Dunkleres Item-Design
        li.className = "flex items-center justify-between bg-slate-950/40 p-3 rounded-lg hover:bg-slate-900/60 transition-all group mb-2 border border-slate-800/60 hover:border-slate-700"; 
        ...
        checkbox.id = task.id;
        // Checkbox: Slate-700 Border, Teal Focus
        checkbox.className = "custom-checkbox h-5 w-5 rounded border-slate-700 focus:ring-teal-500/30 mr-3 shrink-0 mt-0.5 cursor-pointer bg-slate-900";
        checkbox.checked = task.checked;
        
        const label = document.createElement('label');
        label.htmlFor = task.id;
        label.className = "text-sm leading-snug pt-[2px] cursor-pointer text-slate-400 group-hover:text-slate-200 transition select-none font-light tracking-wide";
        ...
        if (task.url && task.url.trim() !== '') {
            ...
            linkBtn.className = "flex items-center justify-center p-1.5 rounded-md bg-slate-900 hover:bg-teal-900/30 text-teal-500/70 hover:text-teal-400 transition shadow-sm border border-slate-800 hover:border-teal-500/30";
...
function createEditableTaskItem(task, index, categoryKey) {
    const li = document.createElement('li');
    // Dunkles Drag Item Design
    li.className = "draggable-item flex items-center group bg-slate-900/80 p-2 rounded border border-transparent hover:border-slate-700 transition gap-2 mb-2";
    li.draggable = true;
    ...
    // Input Style: Darker
    input.className = "task-edit-input flex-grow min-w-0 bg-transparent text-sm rounded px-2 py-1 border border-transparent focus:border-slate-700 focus:bg-slate-950 outline-none text-slate-300 placeholder-slate-600 font-light";
    ...
    // URL Input: Darker
    urlInput.className = "task-url-input w-24 bg-slate-950 text-xs rounded px-2 py-1 border border-slate-800 outline-none text-teal-500 placeholder-slate-700 focus:border-teal-500/50";
    urlInput.dataset.taskId = task.id;

    const delBtn = document.createElement('button');
    delBtn.className = "action-icon-btn delete-task-btn flex-none p-1 hover:text-red-400";
...
function renderEditModal(data) {
    ...
    Object.keys(data).forEach(key => {
        ...
        // Spalten Hintergrund: Sehr dunkel für "eingelassenen" Look
        col.className = 'bg-slate-950/50 p-5 rounded-xl border border-slate-800 flex flex-col h-full min-h-0'; 
        ...
        headerInput.className = "category-title-input font-medium text-slate-200 bg-transparent border-b border-transparent focus:border-teal-500 outline-none mb-3 pb-1 text-sm w-full focus:bg-slate-900 px-1 rounded transition tracking-wide";
        headerInput.dataset.category = key;
        col.appendChild(headerInput);

        // --- ZEIT-STEUERUNG UI ---
        const timeControlDiv = document.createElement('div');
        timeControlDiv.className = "mb-4 p-3 bg-slate-900/50 rounded-lg border border-slate-800 flex flex-col gap-2";
        
        const toggleLabel = document.createElement('label');
        toggleLabel.className = "flex items-center gap-2 cursor-pointer text-[10px] text-slate-500 uppercase tracking-wider select-none hover:text-slate-400 transition";
        const toggleCheck = document.createElement('input');
        toggleCheck.type = "checkbox";
        toggleCheck.className = "accent-teal-600 w-3 h-3 rounded category-auto-select-toggle bg-slate-950 border-slate-700";
        toggleCheck.checked = category.autoSelect || false;
        toggleCheck.dataset.category = key;
        
        toggleLabel.appendChild(toggleCheck);
        toggleLabel.appendChild(document.createTextNode("Auto-Select Zeitfenster"));
        timeControlDiv.appendChild(toggleLabel);

        const inputsDiv = document.createElement('div');
        inputsDiv.className = `flex items-center gap-2 transition-all overflow-hidden ${category.autoSelect ? 'max-h-12 opacity-100 mt-1' : 'max-h-0 opacity-50'}`;
        
        const startInp = document.createElement('input');
        startInp.type = "time";
        startInp.value = category.startTime || "08:00";
        startInp.className = "category-time-start bg-slate-950 border border-slate-700 rounded px-2 py-1 text-xs text-slate-300 focus:border-teal-500 outline-none w-20 text-center";
        startInp.dataset.category = key;

        const endInp = document.createElement('input');
        endInp.type = "time";
        endInp.value = category.endTime || "18:00";
        endInp.className = "category-time-end bg-slate-950 border border-slate-700 rounded px-2 py-1 text-xs text-slate-300 focus:border-teal-500 outline-none w-20 text-center";
        endInp.dataset.category = key;

        inputsDiv.innerHTML = `<span class="text-[10px] text-slate-600 w-4">Von</span>`;
        inputsDiv.appendChild(startInp);
        inputsDiv.innerHTML += `<span class="text-[10px] text-slate-600 w-4 text-center">-</span>`;
        ...
        toggleCheck.addEventListener('change', (e) => {
            if(e.target.checked) {
                ...
                inputsDiv.classList.add('max-h-12', 'opacity-100', 'mt-1');
            } else {
                inputsDiv.classList.add('max-h-0', 'opacity-50');
                inputsDiv.classList.remove('max-h-12', 'opacity-100', 'mt-1');
            }
        });
        ...
        footer.className = "flex-none mt-auto pt-3 border-t border-slate-800 flex gap-2";
        const addInput = document.createElement('input');
        addInput.type = "text";
        addInput.placeholder = "Neuer Task...";
        addInput.className = "flex-grow bg-slate-900 text-xs rounded px-3 py-2 focus:ring-1 focus:ring-teal-500 border border-slate-800 outline-none text-slate-300 placeholder-slate-600";
        const addBtn = document.createElement('button');
        addBtn.textContent = "+";
        addBtn.className = "flex-none bg-teal-800 hover:bg-teal-700 text-teal-100 font-bold py-1 px-3 rounded transition text-xs border border-teal-700";
