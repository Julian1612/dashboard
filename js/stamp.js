...
// Sanftes "Aufwecken" - Meditations Gong / Singing Bowl
...
const leftActions = document.getElementById('stamp-left-actions'); // Container unter dem Timer
...
function handleActionClick() {
    ...
    if (currentStep === 0) {
        ...
        // Modal für Musik-Abfrage manuell bauen
...
function loadStep(stepIndex) {
    ...
    // Reset Left Actions (Button entfernen bei Phasenwechsel)
...
function renderCheckIn() {
    ...
    contentArea.innerHTML = `
        <div class="space-y-6 text-center animate-fade-in">
            <div class="space-y-4">
                <label class="text-xs font-bold text-slate-300 uppercase tracking-wide block">Gefühls-Barometer</label>
                <div class="px-2 pt-2 pb-6 relative">
                    <style>#overwhelm-slider::-webkit-slider-thumb { -webkit-appearance: none; appearance: none; width: 22px; height: 22px; border-radius: 50%; background: white; cursor: pointer; border: 3px solid #1e293b; box-shadow: 0 4px 6px rgba(0,0,0,0.3); margin-top: -9px; transition: transform 0.1s; } #overwhelm-slider::-webkit-slider-thumb:hover { transform: scale(1.1); } #overwhelm-slider::-webkit-slider-runnable-track { width: 100%; height: 6px; cursor: pointer; border-radius: 3px; }</style>
                    <input type="range" id="overwhelm-slider" min="0" max="100" value="50" step="0.1" class="w-full h-2 rounded-full appearance-none cursor-pointer outline-none" style="background: linear-gradient(to right, #22c55e, #eab308, #ef4444);">
                    <div class="flex justify-between text-[10px] text-slate-400 font-bold px-1 mt-3 absolute w-full left-0 -bottom-1 uppercase tracking-wider"><span class="text-green-500">Klar</span><span class="text-red-500">Überfordert</span></div>
                </div>
            </div>
            <div class="p-4 bg-slate-700/30 border border-slate-600/50 rounded-xl"><p class="text-slate-200 text-xs font-bold uppercase tracking-wide mb-2 flex justify-center items-center gap-2"><span class="text-xl">⚡️</span> Physischer Reset</p><p class="text-[11px] text-slate-400 leading-relaxed">Atme tief ein. Zieh die Schultern hoch.<br>Ball die Fäuste. <b>Halten...</b><br>Und locker lassen.</p></div>
        </div>
    `;
}
...
function renderTune() {
    ...
    contentArea.innerHTML = `
        <div class="space-y-4 animate-fade-in">
            <div class="bg-blue-900/20 border border-blue-800/50 p-4 rounded-xl space-y-3"><p class="text-xs font-bold text-blue-400 border-b border-blue-800/50 pb-2 uppercase tracking-wide">Die M.V.P. Formel</p><ul class="text-[11px] text-slate-300 space-y-2"><li class="flex gap-3"><span class="font-bold text-blue-400 bg-blue-900/40 w-5 h-5 flex items-center justify-center rounded text-[10px]">M</span> <span><b>Micro:</b> "5 Minuten"</span></li><li class="flex gap-3"><span class="font-bold text-blue-400 bg-blue-900/40 w-5 h-5 flex items-center justify-center rounded text-[10px]">V</span> <span><b>Verb:</b> "Tippen"</span></li><li class="flex gap-3"><span class="font-bold text-blue-400 bg-blue-900/40 w-5 h-5 flex items-center justify-center rounded text-[10px]">P</span> <span><b>Präzision:</b> "Datei XY"</span></li></ul></div>
            <div><input type="text" id="session-goal-input" autocomplete="off" placeholder="Dein M.V.P. Ziel..." class="w-full bg-slate-700/50 text-white p-3.5 rounded-xl border border-slate-600 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none placeholder-slate-500 text-sm transition-all shadow-inner"><p class="text-[10px] text-slate-500 mt-2 text-center italic opacity-70">Bsp: "10 Min. (M) Stichpunkte tippen (V) in Konzept.docx (P)"</p></div>
        </div>
    `;
...
function renderAtomic() {
    ...
    contentArea.innerHTML = `
        <div class="space-y-5 animate-fade-in">
            <div class="p-3 bg-slate-700/40 border border-slate-600/80 rounded-xl transition-all hover:bg-slate-700/60"><label class="flex items-start gap-3 cursor-pointer group select-none"><div class="relative flex items-center mt-0.5"><input type="checkbox" id="setup-ready-check" class="peer h-5 w-5 cursor-pointer appearance-none rounded border border-slate-500 bg-slate-800 transition-all checked:border-green-500 checked:bg-green-500 checked:scale-110"><svg class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-3.5 h-3.5 pointer-events-none opacity-0 peer-checked:opacity-100 text-white transition-opacity duration-300" viewBox="0 0 14 14" fill="none"><path d="M3 8L6 11L11 3.5" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></div><div class="flex-1"><p class="text-xs font-bold text-white group-hover:text-green-400 transition-colors uppercase tracking-wide">System-Start Pflicht</p><p class="text-[11px] text-slate-400 leading-snug mt-1 transition-colors group-hover:text-slate-300">Ich habe <b>jetzt</b> alle Programme und Tabs geöffnet. Alles liegt bereit.</p></div></label></div>
            <div><label class="text-[10px] uppercase font-bold text-purple-400 block mb-2 tracking-wide">Der allererste Schritt</label><input type="text" id="atomic-step-input" value="${savedBridge}" autocomplete="off" placeholder="Winziger Handgriff (< 30 sek)..." class="w-full bg-slate-700/50 text-white p-3.5 rounded-xl border border-slate-600 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none text-sm transition-all shadow-inner">${savedBridge ? '<p class="text-[10px] text-green-400 mt-2 flex items-center justify-center gap-1 opacity-80"><span class="text-xs">↺</span> Aus "The Bridge" übernommen</p>' : ''}</div>
        </div>
    `;
    setTimeout(() => {
...
function renderMomentum() {
    ...
    contentArea.innerHTML = `
        <div class="space-y-4 h-full flex flex-col animate-fade-in relative">
            <div class="bg-slate-700/30 p-4 rounded-xl border-l-4 border-blue-500 shadow-sm"><p class="text-[10px] uppercase text-slate-500 font-bold tracking-widest mb-1">Session Ziel</p><p class="text-sm font-semibold text-white line-clamp-2 leading-relaxed tracking-tight">${sessionData.goal}</p></div>
            <div class="bg-slate-700/30 p-4 rounded-xl border-l-4 border-purple-500 shadow-sm opacity-80"><p class="text-[10px] uppercase text-slate-500 font-bold tracking-widest mb-1">Erster Schritt</p><p class="text-xs text-white line-clamp-1 font-mono">${sessionData.atomicStep}</p></div>
    ...
    // NEU: Button in die linke Spalte (unter den Timer) rendern!
    leftActions.innerHTML = `
        <button id="activate-focus-btn" class="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 border border-slate-600 px-3 py-1.5 rounded-full text-[10px] font-bold text-slate-300 transition-all shadow-lg hover:scale-105 active:scale-95 group uppercase tracking-wider animate-fade-in">
